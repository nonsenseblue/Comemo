...
cp "$PROJECT_ROOT/src/js/popup.js" "$DIST_DIR/CoMemo/src/js/"
cp "$PROJECT_ROOT/src/css/content.css" "$DIST_DIR/CoMemo/src/css/"

# popup
mkdir -p "$DIST_DIR/CoMemo/popup"
cp "$PROJECT_ROOT/popup/index.html" "$DIST_DIR/CoMemo/popup/"

# Update manifest to use config.example.js
sed -i '' 's/config\.js/config.example.js/g' "$DIST_DIR/CoMemo/manifest.json"
